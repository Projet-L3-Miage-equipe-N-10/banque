...
public class AccountListCtl extends BaseCtl {
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("AccountListCtl populateBean method start");
		AccountBean bean = new AccountBean();
		bean.setAcc_No(DataUtility.getLong(request.getParameter("accNo")));
		log.debug("AccountListCtl populateBean method end");
		return bean;
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				ServletUtility.redirect(BSView.ACCOUNT_LIST_CTL, request, response);
				return;
	...
	protected String getView() {
		return BSView.ACCOUNT_LIST_VIEW;
	}
}
